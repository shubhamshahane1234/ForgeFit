...
export default function Trainers() {
  return (
    <section id="trainers" className="trainers" aria-labelledby="trainers-heading">
      <div className="container">
        ...
        >
          ...
          <h2 id="trainers-heading">Meet Your <span className="accent">Coaches</span></h2>
          <p className="section-sub">Certified experts who live and breathe fitness — and genuinely care about your results.</p>
        </motion.div>
        ...
        <div className="trainers__grid">
          {TRAINERS.map((t, i) => (
            <motion.article
              ...
              transition={{ delay: i * 0.14, duration: 0.6, ease: [.22,1,.36,1] }}
            ...
            >
              ...
              <h3 id={`trainer-${t.id}-name`} className="trainer-card__name">{t.name}</h3>
              <p className="trainer-card__role">{t.role}</p>
              <p className="trainer-card__bio">{t.bio}</p>

              <div className="trainer-card__tags">
                {t.specialty.map(s => (
                  <span key={s} className="trainer-card__tag">{s}</span>
                ))}
              </div>

              <a href="#contact" className="btn btn--ghost trainer-card__btn">Book Session</a>
            </motion.article>
          ))}
        </div>
      </div>
